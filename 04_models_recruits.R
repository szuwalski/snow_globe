...
unc_rec<-read.csv("data/uncertainty_rec.csv")
...
for(y in 1:length(use_stocks))
{
  ...
  if(y==3) # gsl
  {
    ...
    mod_dat<-merge(mod_dat,imm_sd,by="Year",all=TRUE)
     mod_dat$p_mort_mat<-1-exp(-mod_dat$M_mat)
    mod_dat$p_mort_imm<-1-exp(-mod_dat$M_imm) 
    colnames(mod_dat)[c(9,11)]<-c("temp","large_scale")
  }  
 ...
 # mod_rec<-gam(data=mod_dat,Recruitment~s(N_mat,k=3)+s(temp,k=3)+s(large_scale,k=3),family=nb(link = "log"))
  mod_rec<-gam(data=mod_dat,Recruitment~s(N_mat,k=3)+s(temp,k=3)+s(large_scale,k=3),family = tw())
  #mod_rec<-gam(data=mod_dat,Recruitment~s(large_scale,k=3),family = tw())
  summary(mod_rec)
  ...
  # conc_cnt<-conc_cnt+1
  dev_expl_m<-c(dev_expl_m,round(summary(mod_rec)$dev,2))
  plotted<-plot(mod_rec,pages=1)
  ...
  # 
  preds<-predict.gam(mod_rec,type='response',se.fit=TRUE)
...
in_col2<-c("#0034c3","#3da550","#ff8f38")
...
mod_mort_cap <- unc_rec %>%
  group_by(stock) %>%
  mutate(up_m = pmin(up_m, 1.5 * max(est_m, na.rm = TRUE))) %>% # The `pmin` function is used here to take the minimum of the two values element-wise, effectively capping the up_m values.
  ungroup()

mort_plot_alt_trans<-ggplot()+
  geom_point(data=filter(unc_rec,Year<2023),aes(x=Year-5,y=est_m),col='darkgrey')+
  geom_errorbar(data=filter(unc_rec,Year<2023),aes(x=Year-5,ymin=dn_m,ymax=up_m),col='grey')+
...
design<-"112
         112
         112"
png("plots/all_rec_all3.png",height=9,width=6,res=350,units='in') 
